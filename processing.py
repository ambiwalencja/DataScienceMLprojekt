...
def create_10_categories(df, column, new_column_name):
    ...
    def map_income(income):
        if income < quantiles[1]:
            return 1
        elif income < quantiles[2]:
            return 2
        elif income < quantiles[3]:
            return 3
        elif income < quantiles[4]:
            return 4
        elif income < quantiles[5]:
            return 5
        elif income < quantiles[6]:
            return 6
        elif income < quantiles[7]:
            return 7
        elif income < quantiles[8]:
            return 8
        elif income < quantiles[9]:
            return 9
        else:
            return 10
    df[new_column_name] = df[column].apply(lambda income: map_income(income))
